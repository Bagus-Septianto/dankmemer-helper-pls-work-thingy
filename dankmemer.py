...
async def on_message(message):
    ...
    if message.author.id == 270904126974590976 and '- repeat order -' in message.content.lower():
        await message.channel.send(message.content)

    # Emoji Match
    # Copy-Paste the emoji that needs to be remembered
    if message.author.id == 270904126974590976 and '- emoji match -' in message.content.lower():
...
async def on_message_edit(message_before, message_after):
    ...
    if message_before.author.id == 270904126974590976 and 'What was the emoji?' in message_after.content:
    ...
    if message_before.author.id == 270904126974590976 and 'What color was next to the word' in message_after.content:
